...
class _TreeFlattener(libcst.CSTVisitor):
    ...
    def on_visit(self, node: libcst.CSTNode) -> bool:
        name = node.name.value if hasattr(node, "name") else type(node).__name__
...
def compute_graph_diff(tree1: libcst.CSTNode, tree2: libcst.CSTNode) -> set[str]:
    ...
    - Ignores node values
    """
    flattener_actual = _TreeFlattener()
    flattener_expected = _TreeFlattener()
