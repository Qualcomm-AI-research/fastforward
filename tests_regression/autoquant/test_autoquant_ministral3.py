...
# The Ministral-3 checkpoints are multimodal: a `ministral3` text decoder paired with a
# `pixtral` vision tower, both wrapped in `Mistral3ForConditionalGeneration`. Autoquant is
# applied to the whole model, while perplexity is measured over text-only inputs.
_HF_MODEL_NAMES = [
...
@pytest.fixture(scope="module", params=_HF_MODEL_NAMES)
def model_name(request: pytest.FixtureRequest) -> str:
    return str(request.param)


@pytest.fixture
def model(model_name: str, device: torch.device) -> Iterator[Mistral3ForConditionalGeneration]:
    m = Mistral3ForConditionalGeneration.from_pretrained(
...
def test_autoquant_ministral3_matches_fp_perplexity(
    ...
    # GIVEN the wikitext-validation perplexity of a full-precision Ministral-3 model
    ...
    ff.autoquantize(
        model,
        output_path=tmp_path / "_autoquantized_ministral3.py",
