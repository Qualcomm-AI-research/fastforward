...
@pytest.mark.slow
@ff.flags.context(ff.strict_quantization, False)
def test_ff_model_to_onnx_export(tmp_path):
    data = torch.randn(32, 10)
    quant_model, activation_quantizers, parameter_quantizers = define_model()
...
def test_export_function(tmp_path, granularity):
    ...
    assert output_model_directory.exists()
    assert (output_model_directory / model_name).with_suffix(".onnx").exists()
    assert (output_model_directory / model_name).with_suffix(".encodings").exists()
